...
export default function Transactions() {
  ...
  const { data: transactions = { entries: [], total: 0 }, isLoading } =
    useQuery({
                  ...
                  .map((entry: any, idx: number) => (
                    <tr key={idx} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-6 py-4 text-sm">
                        <span
                          className={`px-2 py-1 rounded text-xs font-medium ${entry.type === "DEBIT" ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
                        >
                          {entry.type}
                        </span>
                      </td>
